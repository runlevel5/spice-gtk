                            ...
                            gpointer user_data)
{
    ...
    /* Reorder empty_cd to end */
    {
        GtkWidget *last = NULL;
        GtkWidget *iter;
        for (iter = gtk_widget_get_first_child(GTK_WIDGET(GTK_BOX(self)));
             iter != NULL;
             iter = gtk_widget_get_next_sibling(iter)) {
            last = iter;
        }
        if (last != priv->empty_cd) {
            gtk_box_reorder_child_after(GTK_BOX(self), priv->empty_cd, last);
        }
    }
