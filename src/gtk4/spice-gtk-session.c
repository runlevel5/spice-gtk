                                                    ...
                                                    gpointer      user_data)
{
    ...
    if (g_strcmp0(out_mime, "x-special/gnome-copied-files") == 0 ||
        g_strcmp0(out_mime, "x-special/mate-copied-files") == 0) {
        data = x_special_copied_files_transform_to_data(s, text, &len);
    } else if (g_strcmp0(out_mime, "UTF8_STRING") == 0) {
        gboolean retry = FALSE;
        data = nautilus_uris_transform_to_data(s, text, &len, &retry);

        if (retry && clipboard_find_atom(s, selection, a_uri_list) != NULL) {
    ...
    } else if (g_strcmp0(out_mime, "text/uri-list") == 0) {
